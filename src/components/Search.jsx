...
export default function Search(){
    ...
    useEffect(() => {
        async function fetchData(){
            ...
            for (const stateDoc of statesSnap.docs) {
                const courseSnap = await getDocs(collection(db, "states", stateDoc.id, "courses"));
    ...
    useEffect(() => {
        ...
        const stateMatches = states.filter((s) => s.name.toLowerCase().includes(term));
        const courseMatches = courses.filter((c) => c.name.toLowerCase().includes(term));
                        ...
                        {query && results.length > 0 && (
                            <ul className="w-full max-w-3xl bg-white text-left text-black rounded shadow max-h-60 overflow-y-auto">
                                {results.map((item, index) => (
                                    ...
                                    >
                                        {item.name} {item.stateId ? <span className="text-xs text-gray-500">(Course)</span> : <span className="text-xs text-gray-500">(State)</span>} 
                                    </li>
                                ))}
                            </ul>
                        )}
